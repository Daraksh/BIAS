...
def IPOT_torch_batch(C, bs, n, m, miu, nu, iteration=100, beta=0.5):
...
def GW_distance(X, Y, p, q, lamda=0.5, iteration=5, OT_iteration=100):
	'''
...
def GW_torch_batch(Cs, Ct, bs, n, m, p, q, beta=0.5, iteration=5, OT_iteration=100):
...
def GW_distance_uniform(X, Y, lamda=1e-1, iteration=5, OT_iteration=100):
	m = X.size(2)
	n = Y.size(2)
	bs = X.size(0)
	p = (torch.ones(bs, m, 1)/m).cuda()
