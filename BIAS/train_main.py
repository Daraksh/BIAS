...
def train_model(label, dataloaders, device, dataset_sizes, model,
                criterion, optimizer, scheduler, num_epochs=20, lambda_reg = 1e-4):#, alpha=1.0, beta=0.8):
    #print('hyper-parameters : {}  beta: {}'.format(alpha, beta))
        ...
        for phase in ['train', 'val']:
            ...
            for n_iter, batch in enumerate(loop):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    loss1 = criterion[1](output[1], label_t)
                    loss2 = criterion[2](output[2], label_t)
                    loss3 = criterion[3](output[2],label_c)#torch.tensor(0)
                    l1_reg = sum(torch.norm(param, 1) for param in model.parameters())
                    l1_reg_loss = lambda_reg * l1_reg
                    loss = loss0 + loss1 * 0.2 + loss2 + 1 * l_got + loss3 * 0.8 + l1_reg_loss
                    '''
                    l2_reg = sum(torch.norm(param) ** 2 for param in model.parameters())
                    l2_reg_loss = lambda_reg * l2_reg
                    loss = loss0 + loss1 * 0.2 + loss2 + 1 * l_got + loss3 * 0.8 + l2_reg_loss
                    '''
                    if phase == 'train':
...
def custom_load(
        ...
        label='No Finding',
        dataset_name='mimic',
        image_dir='/DATA1/physionet.org/files/mimic-cxr-jpg/2.1.0/',
...
if __name__ == '__main__':
    ...
    if dev_mode == "dev":
        ...
        df = pd.read_csv("/DATA1/physionet.org/xray_training_dataset_patch.csv").sample(20000)
    else:
