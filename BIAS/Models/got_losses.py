...
class ViTModel_pre_trained(nn.Module):
    def __init__(self, patch_size=16):
        super(ViTModel_pre_trained, self).__init__()
        if patch_size==16:
...
class Network(torch.nn.Module):
    def __init__(self, choice='vgg16', output_size=9, pretrained=True, num_multitask_classes=114) :
        '''
        ...
        if self.choice == 'disentangle':
            ...
            # branch 1
            self.branch_1 = nn.Linear(self.feature_extractor.config.hidden_size, output_size[0])
            # branch 2
            self.branch_2 = nn.Linear(self.feature_extractor.config.hidden_size, output_size[1])

        if self.choice == 'sparse':
            self.feature_extractor = ViTModel.from_pretrained('google/vit-base-patch16-224-in21k')
            self.a = 197
            #self.b = 115
            #self.b = 237057#what should it be 
            self.b = 14
        ...
        if self.choice == 'sparse_multitask':
            self.feature_extractor = ViTModel.from_pretrained('google/vit-base-patch16-224-in21k')
            self.a = 197
            #self.b = 115
            self.b = 237057
            ...
            # branch 2
            self.branch_2 = nn.Linear(self.feature_extractor.config.hidden_size, output_size[1])
            self.multilabelclassifier=MultiLabelClassifier(self.feature_extractor.config.hidden_size, 512,num_multitask_classes)
    ...
    def forward(self, x, attribute=None):
        if self.choice == 'disentangle':
            ...
            return [torch.mean(out_1, dim=1), torch.mean(out_2, dim=1), torch.mean(out_3, dim=1), feature_map]
            # return [out_1, out_2, out_3]

        elif self.choice == 'sparse':
            feature_map = self.feature_extractor(x)  # (bs, bottle_neck)
            feature_map = feature_map.last_hidden_state
            print("size of feature map", feature_map.size())

            x = feature_map
            x = x.view(x.size(0), -1)
            out_mask = self.mask(x)
            print("size of out_mask",out_mask.size())
            #x = torch.sum(out_mask.view(-1, 197, 115),dim=2).unsqueeze(dim=2)
            x = torch.sum(out_mask.view(-1, 197,  14),dim=2).unsqueeze(dim=2)
            ...
            feature_map_detach = feature_map.detach()
            out_3 = self.branch_2(feature_map_detach)
            #print("feature map.shape: ",feature_map.shape, x.shape)
 
     
            
           # return [torch.mean(out_1, dim=1), torch.mean(out_2, dim=1), torch.mean(out_3, dim=1), out_mask.view(-1, 197, 115), feature_map*x]
            return [torch.mean(out_1, dim=1), torch.mean(out_2, dim=1), torch.mean(out_3, dim=1), out_mask.view(-1, 197, 14), feature_map*x]
        ...
        #     return [torch.mean(out_1, dim=1), torch.mean(out_2, dim=1), torch.mean(out_3, dim=1), out_mask.view(-1, 197, 115), feature_map]
        #     # return [out_1, out_2, out_3]
        
        elif self.choice == 'sparse_multitask':
            feature_map = self.feature_extractor(x)  # (bs, bottle_neck)
            feature_map = feature_map.last_hidden_state
            out_1 = self.branch_1(feature_map)
            out_2 = self.branch_2(feature_map)
            ...
            x = feature_map
            x = x.view(x.size(0), -1)
            out_multitask = self.multilabelclassifier(torch.mean(feature_map,dim=1))
            out_mask = self.mask(x)
            return [torch.mean(out_1, dim=1), torch.mean(out_2, dim=1), torch.mean(out_3, dim=1), out_mask.view(-1, 197, 115), feature_map, out_multitask]
            return [torch.mean(out_1, dim=1), torch.mean(out_2, dim=1), torch.mean(out_3, dim=1), out_mask.view(-1, 197, 237057), feature_map, out_multitask]
